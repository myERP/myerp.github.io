...
p(title). "Customer Contacts":customer_contact/#format

p(caption). Customer contacts belongs to a particular customer. The _account_id_ in the URI defines which customer.

|_(thead first). Type |_(thead). Description |
|"GET customers/:account_id/contacts/:id":customer_contact/#customer_contact_get|Returns all the information about the account contact. This contact should belongs to the account _account_id_.|
|"PUT customers/:account_id/contacts/:id":customer_contact/#customer_contact_put|Update a single account contact. This contact should belongs to the account _account_id_.|
|"DELETE customers/:account_id/contacts/:id":customer_contact/#customer_contact_delete|Deletes a single account contact. This contact should belongs to the account _account_id_.|
|"GET customers/:account_id/contacts":customer_contact/#customer_contacts_get|Returns all the contacts that belongs to the account _account_id_.|
|"POST customers/:account_id/contacts":customer_contact/#customer_contacts_post|Create a single account contact. This contact will belongs to the account _account_id_.|
|"PUT customers/:account_id/contacts":customer_contact/#customer_contacts_put|Create and Update several account contacts in a row. All should or will belongs to the account _account_id_.|
|"DELETE customers/:account_id/contacts":customer_contact/#customer_contacts_delete|Delete several account contacts in a row. Those contacts should belongs to the account _account_id_.|
...
p(title). "Accounting transactions":accounting_transaction/#format
...
|_(thead first). Type |_(thead). Description |
|"GET ../accounting_transactions/:id":accounting_transaction/#accounting_transaction_get|Returns all the information about the accounting transaction. |
|"DELETE ../order_lines/:id":accounting_transaction/#accounting_transaction_delete|Deletes a single accounting transaction. Only manual accounting transactions can be deleted.
In order to delete other transactions, you need to deleted the related document (Invoice, Payment, etc...). |
|"GET ../accounting_transactions":accounting_transaction/#accounting_transactions_get|Returns the list of all your accounting transactions. |
|"POST ../accounting_transactions":accounting_transaction/#accounting_transactions_post|Create a single accounting transaction. |
|"DELETE ../accounting_transactions":accounting_transaction/#accounting_transactions_delete|Delete several accounting transactions in a row. Only manual accounting transactions can be deleted.
In order to delete other transactions, you need to delete the related document (Invoice, Payment, etc...). |


h3. Read only resources

p(title). "Chart of Accounts":chart_of_accounts/#format

p(caption). Chart of Accounts is the list of all accounting accounts. Non family accounts can be use to input "accounting transactions":/docs/1.0/api/accounting_transaction/#format.

|_(thead first). Type |_(thead). Description |
|"GET chart_of_accounts/:id":chart_of_accounts/#chart_of_account_get|Returns infos about a particular account from the Chart of Accounts.|
|"GET chart_of_accounts":chart_of_accounts/#chart_of_accounts_get|Returns the whole Chart of Accounts.|
